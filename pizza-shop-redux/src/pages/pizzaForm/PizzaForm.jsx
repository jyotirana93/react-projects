...
import { getAllPizzas } from "../../actions/pizzas.action";
...
const PizzaForm = () => {
  ...
  const storedID = JSON.parse(localStorage.getItem("pizzaID"));
  const { type, name, size, base } = usePizzaOptions(pizzaForm);
  const { createPizzaID } = useID();
  const dispatch = useDispatch();

  const hasActiveOrders = pizzaData.filter(
    (data) => data.stage !== "order-picked" && data.stage !== "cancelled",
  ).length;

  const maxNumberOfPizzasAllowed = 10;
  const canbeOrdered = maxNumberOfPizzasAllowed - hasActiveOrders;
  ...
  useEffect(() => {
    ...
    if (pizzaData?.length > 0) {
      dispatch(getAllPizzas(pizzaData));
    }
  }, [pizzaData]);
  ...
  return (
    <>
      <section>
        {!canbeOrdered && (
          <h3 style={{ color: "red" }}>Not taking any order for now</h3>
        )}
        <form onSubmit={submitHandler}>
          <div className="pizza-form">
            ...
            <button disabled={!canbeOrdered} type="submit">
              Submit
            </button>
          </div>
        </form>
